...
# MONTE CARLO SIMULATION (100 REPLICATIONS)
...
N <- 1000            # Number of sample observations per replication
P <- 20              # Number of original covariates
K_TRUE <- 3          # Ground-truth number of clusters
R <- 100             # 100 Monte Carlo replications
SEED_BASE <- 20260822


############################################################
# 3. NON-GAUSSIAN COPULA DATA GENERATING MECHANISM
############################################################

# Clayton-type frailty copula generator for non-Gaussian tail dependencies
...
# Generate synthetic high-dimensional dataset with ground-truth cluster labels
generate_clustering_data <- function(n = N, p = P, k = K_TRUE) {
  # Assign ground truth clusters balanced across observations
  true_labels <- sample(rep(1:k, length.out = n))
  ...
  # Inject cluster-specific mean shifts and non-linear dependencies
  for (cl in 1:k) {
    idx <- which(true_labels == cl)
    n_cl <- length(idx)
    
    # Cluster center shift
    center_shift <- (cl - 2) * 1.5
    X_sub <- MASS::mvrnorm(n = n_cl, mu = rep(center_shift, p), Sigma = Sigma)
    
    # Non-Gaussian copula distortion on first two dimensions
    cop_u <- copula_latent(n_cl, theta = 1.2 + 0.5 * cl)
    X_sub[, 1] <- X_sub[, 1] + qnorm(cop_u[, 1])
    X_sub[, 2] <- X_sub[, 2] + qnorm(cop_u[, 2])
  ...
  # Pseudo-treatment / structural binary variable T (median split on PC1)
  pc1 <- prcomp(X, scale. = TRUE)$x[, 1]
  T_vec <- ifelse(pc1 > median(pc1), 1, 0)
...
evaluate_clustering <- function(clusters, ground_truth, X_space, runtime) {
  ...
  ari <- mclust::adjustedRandIndex(clusters, ground_truth)
  
  # Silhouette Score
  d_dist <- dist(X_space)
  sil_obj <- cluster::silhouette(clusters, d_dist)
...
cat("============================================================\n")
cat(sprintf("STARTING %d MONTE CARLO CLUSTERING SIMULATIONS\n", R))
cat("============================================================\n")
...
for (r in 1:R) {
  ...
  # Distinct seed for each replication
  set.seed(SEED_BASE + r)
  dat <- generate_clustering_data(n = N, p = P, k = K_TRUE)
...
cat("\n============================================================\n")
cat(sprintf("MONTE CARLO SUMMARY TABLE (%d REPLICATIONS)\n", R))
cat("============================================================\n\n")
...
p_ari <- ggplot(results_df, aes(x = reorder(Method, -ARI, FUN = median), y = ARI, fill = Method)) +
  ...
  labs(
    title = sprintf("ARI Distribution Across 100 Monte Carlo Runs (NP-CTNN Space)"),
...
############################################################
# 3. NON-GAUSSIAN COPULA TIME-SERIES GENERATING MECHANISM
############################################################

generate_forecasting_data <- function(n_time = 1500, p_lags = 10) {
  t_vec <- 1:n_time
  
  trend <- 0.005 * t_vec
  seasonality <- 2.5 * sin(2 * pi * t_vec / 50) + 1.5 * cos(2 * pi * t_vec / 12)
  regime_T <- ifelse(sin(2 * pi * t_vec / 300) > 0, 1, 0)
  
  innovations <- numeric(n_time)
  u_prev <- 0.5
  for (t in 1:n_time) {
    u_curr <- pmin(pmax(0.6 * u_prev + 0.4 * runif(1), 1e-5), 1 - 1e-5)
    innovations[t] <- qnorm(u_curr) * (1 + 0.5 * regime_T[t])
    u_prev <- u_curr
  }
  
  Y <- numeric(n_time)
  Y[1:p_lags] <- rnorm(p_lags)
  
  for (t in (p_lags + 1):n_time) {
    lag_1 <- Y[t - 1]
    lag_2 <- Y[t - 2]
    
    if (regime_T[t] == 1) {
      Y[t] <- trend[t] + seasonality[t] + 0.6 * lag_1 - 0.3 * (lag_2^2) / (1 + abs(lag_2)) + innovations[t]
    } else {
      Y[t] <- trend[t] + seasonality[t] + 0.3 * lag_1 + 0.4 * sin(lag_2) + innovations[t]
    }
  }
...
cat("============================================================\n")
cat(sprintf("STARTING %d MONTE CARLO FORECASTING SIMULATIONS\n", N_SIMS))
cat("============================================================\n")
...
for (s in 1:N_SIMS) {
  ...
  # Set distinct seed for each Monte Carlo replication
  set.seed(SEED_BASE + s)
  
  ts_data <- generate_forecasting_data(n_time = N_TIME, p_lags = P_LAGS)
...
p_rmse_mc <- ggplot(all_mc_results, aes(x = reorder(Model, RMSE, FUN = median), y = RMSE, fill = Model)) +
  ...
  labs(
    title = sprintf("Monte Carlo RMSE Distribution (%d Runs)", N_SIMS),
