...
extract_threshold_value <- function(
    object,
    candidates = c("H", "threshold", "control_limit", "control.limit")) {
  if (is.list(object)) {
...
generate_t <- function(n, delta = 0, df = 5) {
  (rt(as.integer(n), df = df) / sqrt(df / (df - 2))) + delta
}

generate_chisq <- function(n, delta = 0, df = 5) {
  ((rchisq(as.integer(n), df = df) - df) / sqrt(2 * df)) + delta
}

generate_lognormal <- function(n, delta = 0, meanlog = 0, sdlog = 0.50) {
  x <- rlnorm(as.integer(n), meanlog = meanlog, sdlog = sdlog)
  mu <- exp(meanlog + 0.5 * sdlog^2)
  variance <- (exp(sdlog^2) - 1) * exp(2 * meanlog + sdlog^2)
  ((x - mu) / sqrt(variance)) + delta
}

generate_laplace <- function(n, delta = 0) {
  b <- 1 / sqrt(2)
  (rexp(as.integer(n), rate = 1 / b) - rexp(as.integer(n), rate = 1 / b)) + delta
}

generate_gaussian_mixture <- function(n, delta = 0, mixing = 0.90, sd1 = 1, sd2 = 4) {
  n <- as.integer(n)
  component <- runif(n) < mixing
  x <- numeric(n)
  n1 <- sum(component)
  n2 <- n - n1
  if (n1 > 0L) x[component] <- rnorm(n1, mean = 0, sd = sd1)
  if (n2 > 0L) x[!component] <- rnorm(n2, mean = 0, sd = sd2)
  var_theory <- mixing * sd1^2 + (1 - mixing) * sd2^2
  (x / sqrt(var_theory)) + delta
}

NONNORMAL_GENERATORS <- list(
  Normal = function(n, delta = 0) generate_normal(n, delta),
  t5 = function(n, delta = 0) generate_t(n, delta, df = 5),
  ChiSquare5 = function(n, delta = 0) generate_chisq(n, delta, df = 5),
  Lognormal = function(n, delta = 0) generate_lognormal(n, delta, meanlog = 0, sdlog = 0.50),
  Laplace = function(n, delta = 0) generate_laplace(n, delta),
  GaussianMixture = function(n, delta = 0) generate_gaussian_mixture(n, delta, mixing = 0.90, sd1 = 1, sd2 = 4)
)
...
extract_sp_ecusum_components <- function(fit) {
  ...
  get_first <- function(object, candidates, required = TRUE) {
    ...
    if (required) {
      stop(paste("Could not find any of:", paste(candidates, collapse = ", ")), call. = FALSE)
    }
    NULL
  }
...
apply_nonnormal_probability_transform <- function(
  ...
  if (method %in% c("empirical", "empirical_copula")) {
    ...
    if (is.function(stationary_model)) {
      return(stationary_model(value))
    } else if (is.list(stationary_model) && !is.null(stationary_model$ecdf)) {
      return(stationary_model$ecdf(value))
    } else if (is.list(stationary_model) && !is.null(stationary_model$empirical_samples)) {
      samples <- stationary_model$empirical_samples
      p_less <- mean(samples < value)
      p_equal <- mean(samples == value)
      return(p_less + 0.5 * p_equal)
  ...
  if (method == "mid" && exists("probability_scale_mid_transform", mode = "function", inherits = TRUE)) {
    return(probability_scale_mid_transform(value, stationary_model))
  }

  if (!exists("probability_scale_transform", mode = "function", inherits = TRUE)) {
    stop("probability_scale_transform() was not found. Source 04_probability_transform.R first.", call. = FALSE)
  }
  ...
  if ("method" %in% fml) {
    out <- f(value, stationary_model, method = method)
  } else if ("transform_method" %in% fml) {
    out <- f(value, stationary_model, transform_method = method)
  } else if ("lower_tail" %in% fml) {
    out <- f(value, stationary_model, lower_tail = identical(method, "lower_tail"))
  } else {
    out <- f(value, stationary_model)
  }

  pmin(pmax(as.numeric(out)[1L], 0), 1)
}
...
simulate_nonnormal_sp_ecusum <- function(
  ...
  transform_method <- match.arg(transform_method, c("mid", "lower_tail", "empirical", "empirical_copula"))
  ...
  for (t in seq_len(max_run)) {
    x <- generator_fn(n = 1L, delta = delta)

    for (j in seq_len(J)) {
      x_update <- if (side == "upper") x else -x
      C[j] <- upper_cusum_update(C_prev = C[j], x = x_update, k = k_values[j])
    }
...
simulate_nonnormal_sp_ecusum_arl <- function(
  ...
  for (r in seq_len(n_rep)) {
    run_lengths[r] <- simulate_nonnormal_sp_ecusum(
...
get_nonnormal_sp_ecusum_fit <- function(config, fit = NULL) {
  config <- normalize_nonnormal_config(config)
  ...
  if (!is.null(fit)) {
    ...
    result$side <- components$side \%\vert{}\vert{}\% config$side
    result$transform_method <- components$transform_method \%\vert{}\vert{}\% config$transform_method
  ...
  for (nm in c("build_empirical_copula_models", "build_stationary_models", "fit_stationary_models")) {
    if (exists(nm, mode = "function", inherits = TRUE)) {
      stationary_builder <- get(nm, mode = "function", inherits = TRUE)
      break
    }
  }

  if (is.null(stationary_builder)) {
    stop("No stationary/empirical copula model builder function found.", call. = FALSE)
  }

  stationary_models <- stationary_builder(k_values = k_values)

  # Standard calibrate_threshold call matching 09_simulation_normal.R
  ...
  H <- extract_threshold_value(calibration)
...
run_nonnormal_simulation <- function(
  ...
  sp_fit <- get_nonnormal_sp_ecusum_fit(config = config, fit = fit)
  sp_components <- extract_sp_ecusum_components(sp_fit)

  dist_names <- names(generators)
  results_list <- vector("list", length(dist_names))

  for (d in seq_along(dist_names)) {
    dname <- dist_names[d]
    gen_fn <- generators[[dname]]

    # ARL0 Simulation
    arl0_rl <- simulate_nonnormal_sp_ecusum_arl(
      generator_fn = gen_fn,
      delta = 0,
      ...
      H = sp_components$H,
      stationary_models = sp_components$stationary_models,
      max_run = config$max_run_arl0,
      side = sp_components$side \%\vert{}\vert{}\% config$side,
      transform_method = sp_components$transform_method \%\vert{}\vert{}\% config$transform_method
    )
    arl0_sum <- summarize_run_lengths(arl0_rl, config$max_run_arl0)
    arl0_sum$distribution <- dname

    # OOC Simulation
    ooc_rows <- vector("list", length(config$shifts))
    for (i in seq_along(config$shifts)) {
      delta <- config$shifts[i]
      ooc_rl <- simulate_nonnormal_sp_ecusum_arl(
        generator_fn = gen_fn,
        delta = delta,
      ...
      sm_ooc <- summarize_run_lengths(ooc_rl, config$max_run_ooc)
      sm_ooc$distribution <- dname
      sm_ooc$shift <- delta
      ooc_rows[[i]] <- sm_ooc
    }
    ...
    results_list[[d]] <- list(
      distribution = dname,
      arl0 = arl0_sum,
      ooc = do.call(rbind, ooc_rows)
    )
  }

  arl0_combined <- do.call(rbind, lapply(results_list, function(x) x$arl0))
  ooc_combined <- do.call(rbind, lapply(results_list, function(x) x$ooc))
  ...
  class(result) <- c("sp_ecusum_nonnormal_simulation", "list")
  result
}