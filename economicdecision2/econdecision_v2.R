...
# =============================================================================
# 18B.1 Configuration validation
# =============================================================================
...
# -----------------------------------------------------------------------------
# Canonical reviewer-required PER sensitivity grid.
# -----------------------------------------------------------------------------
...
# Override any legacy DQN/PER configuration.
PER_ALPHA_GRID <- required_alpha_grid

# Reference/default alpha only.
# The actual analysis evaluates every value in PER_ALPHA_GRID.
PER_ALPHA <- 0.50
...
# Retained for backward-compatible configuration reporting.
# It is NOT used because no importance-sampling correction is implemented.
if (!exists("PER_BETA")) {
  PER_BETA <- 0.40
}
...
if (
  length(PER_EPSILON) != 1L ||
  !is.finite(PER_EPSILON) ||
  PER_EPSILON <= 0
) {
  stop(
    "PER_EPSILON must be a single positive finite value."
  )
}
...
# =============================================================================
# 18B.3 Reproducibility
# =============================================================================

set_bandit_seed <- function(
    seed = BANDIT_SEED
) {
...
# =============================================================================
# 18B.4 TensorFlow input conversion
#
# IMPORTANT:
# Keras 3 through reticulate may reject an ordinary R matrix passed directly
# to model$fit() or predict(). All model inputs are therefore explicitly
# converted to TensorFlow float32 tensors.
# =============================================================================

as_tf_float32 <- function(
    x
) {
...
validate_bandit_data <- function(
  ...
  if (
    ...
    stop(
      "The number of actions does not match the number of contexts."
    )
  }
  ...
  if (
    ...
    stop(
      "Actions must belong to ACTION_VALUES."
    )
  }
...
# =============================================================================
# 18B.6 Training-only standardization
# =============================================================================
...
apply_bandit_scaler <- function(
  ...
  if (
    ...
    stop(
      "X must be a three-dimensional array."
    )
  }
...
make_action_input <- function(
  ...
  if (
    length(A) != nrow(X_flat)
  ) {
    stop(
      "Action length does not match X."
    )
  }
...
# =============================================================================
# 18B.8 CNN-LSTM input construction
# =============================================================================
...
sample_per_indices <- function(
  ...
  if (n == 0L) {
    stop(
      "No observations available for sampling."
    )
  }
...
build_bandit_cnn_lstm <- function(
  ...
  outputs <-
    ...
    keras::layer_conv_1d(
      ...
      kernel_size = CNN_LSTM_KERNEL_SIZE,
      activation = "relu",
      padding = "same"
    ) |>
    keras::layer_lstm(
...
fit_bandit_mlp <- function(
  ...
  for (step in seq_len(n_updates)) {
    ...
    x_batch_tf <- as_tf_float32(
      x_batch
    )
    
    y_batch_tf <- as_tf_float32(
      y_batch
    )
    ...
    priorities <- update_bandit_priorities(
      priorities = priorities,
      indices = idx,
      rewards = reward_train[idx],
...
fit_bandit_cnn_lstm <- function(
  ...
  X_input <- make_cnn_lstm_action_input(
    X_train,
    A_train
  )
  ...
  model <- build_bandit_cnn_lstm(
    lookback = dim(X_input)[2L],
    n_features = dim(X_input)[3L]
  )
...
predict_bandit_cnn_lstm <- function(
  ...
  if (
    ...
    stop(
      "CNN-LSTM prediction input must be three-dimensional."
    )
  }
...
# =============================================================================
# 18B.17 One-step policy evaluation for MLP
# =============================================================================
...
evaluate_one_step_policy <- function(
  ...
  X0 <- make_action_input(
    X,
    rep(0, n)
  )
  
  X1 <- make_action_input(
    X,
    rep(1, n)
  )
  
  mu0 <- predict_bandit_mlp(
    model,
    X0
  )
  
  mu1 <- predict_bandit_mlp(
    model,
    X1
  )
...
# =============================================================================
# 18B.18 Reward prediction evaluation
# =============================================================================
...
# =============================================================================
# 18B.19 Policy metrics
# =============================================================================

calculate_policy_metrics <- function(
    policy_action,
    predicted_reward,
    observed_action,
    observed_reward,
...
run_mlp_alpha <- function(
  ...
  if (
    length(train_idx) == 0L ||
    length(validation_idx) == 0L ||
    length(test_idx) == 0L
  ) {
    stop(
      "Train, validation, and test splits must all be nonempty."
    )
  }
  
  X_train_raw <- bandit_data$X[
    train_idx,
    ,
    ,
    drop = FALSE
  ...
  X_validation_raw <- bandit_data$X[
    validation_idx,
    ,
    ,
    drop = FALSE
  ...
  model <- fit_bandit_mlp(
    X_train = X_train,
    A_train = bandit_data$A[
      train_idx
    ],
    reward_train = bandit_data$reward[
      train_idx
    ],
    alpha = alpha,
    seed = seed
  ...
  validation_policy <- evaluate_one_step_policy(
    model = model,
    X = X_validation,
    policy_cost = policy_cost
  )
  
  validation_metrics <- calculate_policy_metrics(
    policy_action =
      validation_policy$policy_action,
    predicted_reward =
      validation_policy$policy_reward,
    observed_action =
      bandit_data$A[
        validation_idx
      ],
  ...
  validation_prediction <- evaluate_reward_prediction(
    model = model,
    X = X_validation,
    A = bandit_data$A[
      validation_idx
  ...
  test_policy <- evaluate_one_step_policy(
    model = model,
    X = X_test,
    policy_cost = policy_cost
  )
  
  test_metrics <- calculate_policy_metrics(
    policy_action =
      test_policy$policy_action,
    predicted_reward =
      test_policy$policy_reward,
    observed_action =
      bandit_data$A[
        test_idx
      ],
  ...
  test_prediction <- evaluate_reward_prediction(
    model = model,
    X = X_test,
    A = bandit_data$A[
      test_idx
...
run_cnn_lstm_ablation <- function(
  ...
  # Fixed uniform-sampling ablation.
  ...
  X_validation_input <- make_cnn_lstm_action_input(
    X_validation,
    bandit_data$A[
      validation_idx
    ]
  ...
  validation_prediction <- predict_bandit_cnn_lstm(
    model,
    X_validation_input
  )
  ...
  # ---------------------------------------------------------------------------
  # Validation policy under A = 0 and A = 1.
  # ---------------------------------------------------------------------------
  
  X_validation_0 <- make_cnn_lstm_action_input(
    X_validation,
    rep(
      0,
  ...
  X_validation_1 <- make_cnn_lstm_action_input(
    X_validation,
    rep(
      1,
      length(validation_idx)
  ...
  mu0_validation <- predict_bandit_cnn_lstm(
    model,
    X_validation_0
  )
  
  mu1_validation <- predict_bandit_cnn_lstm(
    model,
    X_validation_1
  )
  
  validation_action <- as.integer(
    (
      mu1_validation -
        policy_cost
    ) >
  ...
  validation_policy_reward <- ifelse(
    validation_action == 1L,
    mu1_validation - policy_cost,
    mu0_validation
  )
  ...
  X_test_input <- make_cnn_lstm_action_input(
    X_test,
    bandit_data$A[
      test_idx
    ]
  ...
  test_prediction <- predict_bandit_cnn_lstm(
    model,
    X_test_input
  )
  ...
  # ---------------------------------------------------------------------------
  # Test policy under A = 0 and A = 1.
  # ---------------------------------------------------------------------------
  
  X_test_0 <- make_cnn_lstm_action_input(
    X_test,
    rep(
      0,
      length(test_idx)
  ...
  X_test_1 <- make_cnn_lstm_action_input(
    X_test,
    rep(
      1,
      length(test_idx)
  ...
  mu0_test <- predict_bandit_cnn_lstm(
    model,
    X_test_0
  )
  
  mu1_test <- predict_bandit_cnn_lstm(
    model,
    X_test_1
  )
  
  test_action <- as.integer(
    (
      mu1_test -
        policy_cost
    ) >
  ...
  test_policy_reward <- ifelse(
    test_action == 1L,
    mu1_test - policy_cost,
    mu0_test
  )
...
# =============================================================================
# 18B.22 Full one-step contextual-bandit pipeline
# =============================================================================
...
run_bandit_per_pipeline <- function(
  ...
  if (!isTRUE(
    ...
    stop(
      paste0(
        "The alpha sensitivity grid must be exactly ",
        "c(0, 0.25, 0.50, 0.75, 1.00)."
      )
    )
  }
  ...
  # ---------------------------------------------------------------------------
  # Evaluate the complete reviewer-required alpha grid.
  # ---------------------------------------------------------------------------
  ...
  for (i in seq_along(alpha_grid)) {
    ...
    if (
      ...
      cat(
        "Sampling mode: PRIORITY-WEIGHTED SAMPLING\n"
      )
    }
    
    mlp_results[[i]] <- run_mlp_alpha(
    ...
    # Do not retain five live Keras models.
    mlp_results[[i]]$model <- NULL
      ...
      function(x) {
        ...
        data.frame(
          ...
          validation_mae =
            x$validation_mae,
          policy_value =
            x$validation$Model_Based_Policy_Value,
          treatment_rate =
            x$validation$Treatment_Rate,
          action_agreement =
            x$validation$Action_Agreement,
          policy_regret =
            x$validation$Policy_Regret,
          stringsAsFactors = FALSE
        )
      }
    )
  ...
  # Higher policy value is preferred.
  # Ties are resolved by lower regret, then lower RMSE.
  # ---------------------------------------------------------------------------
  
  validation_order <- order(
    -validation_table$policy_value,
    validation_table$policy_regret,
    validation_table$validation_rmse
  )
  
  selected_row <- validation_order[1L]
  
  selected_alpha <- validation_table$alpha[
    selected_row
  ]
  
  # ---------------------------------------------------------------------------
  # Refit selected primary MLP in the active Keras session.
  # ---------------------------------------------------------------------------
  ...
  cat("\n")
  cat("------------------------------------------------------------\n")
  cat("CNN-LSTM SEQUENCE-MODEL ABLATION\n")
  cat("Sampling mode: EXACT UNIFORM SAMPLING\n")
  
  clear_bandit_session()
  
  cnn_lstm_result <- run_cnn_lstm_ablation(
    bandit_data = bandit_data,
    policy_cost = policy_cost,
    seed = seed
  )
  
  cnn_lstm_result$model <- NULL
  ...
  model_comparison <- rbind(
    ...
    data.frame(
      ...
      alpha = selected_alpha,
      validation_rmse =
        selected_mlp$validation_rmse,
      validation_mae =
        selected_mlp$validation_mae,
      validation_policy_value =
        selected_mlp$validation$Model_Based_Policy_Value,
      test_rmse =
        selected_mlp$test_rmse,
      test_mae =
        selected_mlp$test_mae,
      test_policy_value =
        selected_mlp$test$Model_Based_Policy_Value,
      test_treatment_rate =
        selected_mlp$test$Treatment_Rate,
      test_action_agreement =
        selected_mlp$test$Action_Agreement,
      test_policy_regret =
        selected_mlp$test$Policy_Regret,
      stringsAsFactors = FALSE
    ),
    ...
    data.frame(
      ...
      alpha = 0.00,
      validation_rmse =
        cnn_lstm_result$validation_rmse,
      validation_mae =
        cnn_lstm_result$validation_mae,
      validation_policy_value =
        cnn_lstm_result$validation$Model_Based_Policy_Value,
      test_rmse =
        cnn_lstm_result$test_rmse,
      test_mae =
        cnn_lstm_result$test_mae,
      test_policy_value =
        cnn_lstm_result$test$Model_Based_Policy_Value,
      test_treatment_rate =
        cnn_lstm_result$test$Treatment_Rate,
      test_action_agreement =
        cnn_lstm_result$test$Action_Agreement,
      test_policy_regret =
        cnn_lstm_result$test$Policy_Regret,
      stringsAsFactors = FALSE
    )
  )
  ...
  # ---------------------------------------------------------------------------
  # Remove live selected model before saving.
  # ---------------------------------------------------------------------------
  
  selected_mlp$model <- NULL
...
# =============================================================================
# 18B.23 Implementation checks
# =============================================================================

cat("\n")
cat("Implementation checks:\n")
...
cat(
  "  CNN-LSTM: ablation only\n"
)

cat(
  "  One-step reward: Y_next - policy_cost * A\n"
)
...
cat("\n")
cat("Section 18B loaded successfully.\n")
